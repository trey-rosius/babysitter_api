...
class TransferCoordinator(object):
    ...
    def __repr__(self):
        return "%s(transfer_id=%s)" % (self.__class__.__name__, self.transfer_id)
    ...
    def add_done_callback(self, function, *args, **kwargs):
        ...
        with self._done_callbacks_lock:
            self._done_callbacks.append(FunctionContainer(function, *args, **kwargs))
...
class NonThreadedExecutor(BaseExecutor):
    ...
    def submit(self, fn, *args, **kwargs):
        ...
        try:
            ...
            e, tb = sys.exc_info()[1:]
            logger.debug(
                "Setting exception for %s to %s with traceback %s", future, e, tb
            )
