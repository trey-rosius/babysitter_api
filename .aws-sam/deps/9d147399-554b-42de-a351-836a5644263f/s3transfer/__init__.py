...
class ReadFileChunk(object):
    ...
    @classmethod
    def from_filename(
        cls, filename, start_byte, chunk_size, callback=None, enable_callback=True
    ):
...
class MultipartUploader(object):
    ...
    def __init__(
        self, client, config, osutil, executor_cls=concurrent.futures.ThreadPoolExecutor
    ):
        self._client = client
        self._config = config
        self._os = osutil
    ...
    def upload_file(self, filename, bucket, key, callback, extra_args):
        ...
        try:
            ...
            logger.debug(
                "Exception raised while uploading parts, " "aborting multipart upload.",
                exc_info=True,
            )
...
class MultipartDownloader(object):
    ...
    def _download_file_as_future(self, bucket, key, filename, object_size, callback):
        ...
        download_partial = functools.partial(
            self._download_range, bucket, key, filename, part_size, num_parts, callback
        )
        try:
            with self._executor_cls(max_workers=max_workers) as executor:
            ...
            while True:
                ...
                if task is SHUTDOWN_SENTINEL:
                    ...
                    try:
                        ...
                        logger.debug(
                            "Caught exception in IO thread: %s", e, exc_info=True
                        )
...
class S3Transfer(object):
    ...
    def _put_object(self, filename, bucket, key, callback, extra_args):
        ...
        with open_chunk_reader(
            filename, 0, self._osutil.get_file_size(filename), callback=callback
        ) as body:
    ...
    def _get_object(self, bucket, key, filename, extra_args, callback):
        ...
        for i in range(max_attempts):
            try:
                return self._do_get_object(bucket, key, filename, extra_args, callback)
            except (
                socket.timeout,
                socket.error,
                ReadTimeoutError,
