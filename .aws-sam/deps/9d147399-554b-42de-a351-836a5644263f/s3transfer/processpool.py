...
class ProcessPoolDownloader(object):
        ...
        for kwarg in provided:
            if kwarg not in ALLOWED_DOWNLOAD_ARGS:
                raise ValueError(
                    ...
                    "must be one of: %s" % (kwarg, ", ".join(ALLOWED_DOWNLOAD_ARGS))
                )
    ...
    def _start_get_object_workers(self):
        logger.debug(
            "Starting %s GetObjectWorkers.", self._transfer_config.max_request_processes
        )
...
class GetObjectWorker(BaseS3TransferProcess):
    ...
    def _do_run(self):
        while True:
            ...
            logger.debug(
                "%s jobs remaining for transfer_id %s.", remaining, job.transfer_id
            )
            if not remaining:
    ...
    def _run_get_object_job(self, job):
        try:
            ...
            logger.debug(
                "Exception caught when downloading object for " "get object job %s: %s",
                job,
                e,
                exc_info=True,
            )
