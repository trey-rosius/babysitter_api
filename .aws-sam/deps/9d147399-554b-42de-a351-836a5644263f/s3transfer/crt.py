    ...
    def _get_credentials(self):
        with self._lock:
            if self._loaded_credentials is None:
                loaded_creds = self._botocore_credential_provider.load_credentials()
...
def create_s3_crt_client(
    ...
    if botocore_credential_provider:
        ...
        provider = AwsCredentialsProvider.new_delegate(credentails_provider_adapter)
    ...
    def set_exception(self, exception):
        ...
        if not self.done():
            raise TransferNotDoneError(
                "set_exception can only be called once the transfer is " "complete."
            )
...
class S3ClientArgsCreator:
    ...
    def get_crt_callback(
        self, future, callback_type, before_subscribers=None, after_subscribers=None
    ):
