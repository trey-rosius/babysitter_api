...
class TransferManager(object):
    ...
    ALLOWED_DELETE_ARGS = ["MFA", "VersionId", "RequestPayer", "ExpectedBucketOwner"]
    ...
    def _submit_transfer(self, call_args, submission_task_cls, extra_main_kwargs=None):
        ...
        self._submission_executor.submit(
            submission_task_cls(
                transfer_coordinator=components["coordinator"], main_kwargs=main_kwargs
            )
        )
    ...
    def _register_handlers(self):
        ...
        self._client.meta.events.register_first(
            event_name, signal_not_transferring, unique_id="s3upload-not-transferring"
        )
    ...
    def wait(self):
        ...
        try:
            ...
            if transfer_coordinator:
                logger.debug(
                    "On KeyboardInterrupt was waiting for %s", transfer_coordinator
                )
            raise
        except Exception:
