    ...
    def queue_file_io_task(self, fileobj, data, offset):
        with self._io_submit_lock:
            ...
            for write in writes:
                data = write["data"]
                logger.debug(
                    "Queueing IO offset %s for fileobj: %s", write["offset"], fileobj
                )
                super(DownloadNonSeekableOutputManager, self).queue_file_io_task(
                    fileobj, data, offset
                )
...
class DownloadSpecialFilenameOutputManager(DownloadNonSeekableOutputManager):
    ...
    @classmethod
    def is_compatible(cls, download_target, osutil):
        return isinstance(download_target, six.string_types) and osutil.is_special_file(
            download_target
        )
...
class DownloadSubmissionTask(SubmissionTask):
    ...
    def _get_download_output_manager_cls(self, transfer_future, osutil):
        ...
        raise RuntimeError(
            "Output %s of type: %s is not supported." % (fileobj, type(fileobj))
        )
            ...
            try:
                ...
                for chunk in chunks:
                    ...
                    if not self._transfer_coordinator.done():
                        self._handle_io(
                            download_output_manager, fileobj, chunk, current_index
                        )
