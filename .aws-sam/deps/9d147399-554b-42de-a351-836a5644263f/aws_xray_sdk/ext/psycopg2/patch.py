...
def patch():
    ...
    wrapt.wrap_function_wrapper("psycopg2", "connect", _xray_traced_connect)
    wrapt.wrap_function_wrapper(
        "psycopg2.extensions", "register_type", _xray_register_type_fix
    )
    wrapt.wrap_function_wrapper(
        "psycopg2.extensions", "quote_ident", _xray_register_type_fix
    )
...
def _xray_traced_connect(wrapped, instance, args, kwargs):
    ...
    parameterized_dsn = {
        c[0]: c[-1] for c in map(methodcaller("split", "="), conn.dsn.split(" "))
    }
    meta = {
...
def _xray_register_type_fix(wrapped, instance, args, kwargs):
    ...
    if len(our_args) == 2 and isinstance(
        our_args[1], (XRayTracedConn, XRayTracedCursor)
    ):
