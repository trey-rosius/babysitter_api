...
class SigV4Auth(BaseSigner):
    ...
    def _canonical_query_string_params(self, params):
        ...
        for key in params:
            ...
            key_val_pairs.append((quote(key, safe="-_.~"), quote(value, safe="-_.~")))
        sorted_key_vals = []
...
class SigV4QueryAuth(SigV4Auth):
    ...
    def __init__(self, credentials, service_name, region_name, expires=DEFAULT_EXPIRES):
        super(SigV4QueryAuth, self).__init__(credentials, service_name, region_name)
    ...
    def _modify_request_before_signing(self, request):
        ...
        query_dict = dict(
            [
                (k, v[0])
                for k, v in parse_qs(url_parts.query, keep_blank_values=True).items()
            ]
        )
        if request.params:
        ...
        new_query_string = operation_params + percent_encode_sequence(auth_params)
