...
class ClientCreator(object):
    ...
    def create_client(
        ...
        self._register_s3_events(
            service_client, endpoint_bridge, endpoint_url, client_config, scoped_config
        )
    ...
    def _register_endpoint_discovery(self, client, endpoint_url, config):
        ...
        if enabled and self._requires_endpoint_discovery(client, enabled):
            ...
            events.register(
                "before-parameter-build", block_endpoint_discovery_required_operations
            )
    ...
    def _register_s3_events(
        self, client, endpoint_bridge, endpoint_url, client_config, scoped_config
    ):
...
class BaseClient(object):
    ...
    def _convert_to_request_dict(self, api_params, operation_model, context=None):
        api_params = self._emit_api_params(api_params, operation_model, context)
    ...
    def _emit_api_params(self, api_params, operation_model, context):
        ...
        self.meta.events.emit(
            event_name.format(service_id=service_id, operation_name=operation_name),
            params=api_params,
            model=operation_model,
            context=context,
        )
    ...
    def get_paginator(self, operation_name):
        ...
        if not self.can_paginate(operation_name):
            ...
            paginator = documented_paginator_cls(
                getattr(self, operation_name), paginator_config, operation_model
            )
            return paginator
    ...
    def get_waiter(self, waiter_name):
        ...
        return waiter.create_waiter_with_client(mapping[waiter_name], model, self)
    ...
    def _load_exceptions(self):
        return self._exceptions_factory.create_client_exceptions(self._service_model)
