...
class Endpoint(object):
    ...
    def make_request(self, operation_model, request_dict):
        logger.debug(
            "Making request for %s with params: %s", operation_model, request_dict
        )
    ...
    def create_request(self, params, operation_model=None):
        ...
        if operation_model:
            ...
            self._event_emitter.emit(
                event_name, request=request, operation_name=operation_model.name
            )
    ...
    def _send_request(self, request_dict, operation_model):
        ...
        while self._needs_retry(
            attempts, operation_model, request_dict, success_response, exception
        ):
            attempts += 1
        ...
        if success_response is not None and "ResponseMetadata" in success_response[1]:
    ...
    def _get_response(self, request, operation_model, context):
        ...
        success_response, exception = self._do_get_response(request, operation_model)
        kwargs_to_emit = {
            "response_dict": None,
...
class EndpointCreator(object):
    ...
    def create_endpoint(
        ...
        if not is_valid_endpoint_url(endpoint_url) and not is_valid_ipv6_endpoint_url(
            endpoint_url
        ):
