    ...
    def traverse_and_document_shape(
        ...
        if shape.name in history:
            ...
            if hasattr(shape, "is_document_type") and shape.is_document_type:
                param_type = "document"
            getattr(
                self, "document_shape_type_%s" % param_type, self.document_shape_default
            )(
                section,
                shape,
                history=history,
            ...
            if is_top_level_param:
                self._event_emitter.emit(
                    "docs.%s.%s.%s.%s"
                    % (self.EVENT_NAME, self._service_name, self._operation_name, name),
                    section=section,
                )
            ...
            if at_overlying_method_section:
                self._event_emitter.emit(
                    ...
                    % (self.EVENT_NAME, self._service_name, self._operation_name),
                    section=section,
                )
            history.pop()
