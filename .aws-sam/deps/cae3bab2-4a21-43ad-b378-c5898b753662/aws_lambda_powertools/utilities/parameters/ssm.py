...
class SSMProvider(BaseProvider):
    ...
    **Retrieves multiple parameter values from Systems Manager Parameter Store passing options to the SDK call**
    ...
    def get(  # type: ignore[override]
        ...
        **sdk_options,
        ...
        max_age = resolve_max_age(
            env=os.getenv(constants.PARAMETERS_MAX_AGE_ENV, DEFAULT_MAX_AGE_SECS),
            choice=max_age,
        )
    ...
    def get_parameters_by_name(
        ...
        self._raise_if_errors_key_is_present(
            parameters, self._ERRORS_KEY, raise_on_error
        )
    ...
    def _get_parameters_by_name_from_cache(
    ...
    def _get_parameters_by_name_in_chunks(
        ...
        for chunk in slice_dictionary(
            data=diff, chunk_size=self._MAX_GET_PARAMETERS_ITEM
        ):
    ...
    def _get_parameters_by_name(
        ...
        batch_errors = self._handle_any_invalid_get_parameter_errors(
        ...
        if failed_parameters:
            if raise_on_error:
                raise GetParameterError(
                    f"Failed to fetch parameters: {failed_parameters}"
                )
