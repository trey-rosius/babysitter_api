...
class _S3SeekableIO(IO[bytes]):
    ...
    def s3_client(self) -> "Client":
        ...
        if not self._has_user_agent:
            user_agent.register_feature_to_client(
                client=self._s3_client, feature="streaming"
            )
    ...
    def size(self) -> int:
        ...
        if self._size is None:
            ...
            self._size = self.s3_client.head_object(**self._sdk_options).get(
                "ContentLength", 0
            )
        return self._size
    ...
    def raw_stream(self) -> PowertoolsStreamingBody:
        ...
        if self._raw_stream is None:
            ...
            self._raw_stream = self.s3_client.get_object(
                Range=range_header, **self._sdk_options
            ).get("Body")
            self._closed = False
    ...
    def write(
        self,
        data: Union[
            bytes, Union[bytearray, memoryview, Sequence[Any], "mmap", "_CData"]
        ],
    ) -> int:
        raise NotImplementedError("this stream is not writable")
    ...
    def writelines(
        self,
        data: Iterable[
            Union[bytes, Union[bytearray, memoryview, Sequence[Any], "mmap", "_CData"]]
        ],
    ) -> None:
        raise NotImplementedError("this stream is not writable")
