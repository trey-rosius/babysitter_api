...
class BasePartialProcessor(ABC):
        ...
        async def async_process_closure():
            return list(
                await asyncio.gather(
                    *[self._async_process_record(record) for record in self.records]
                )
            )
    ...
    def __call__(
        ...
        if lambda_context is None:
            ...
            self._handler_accepts_lambda_context = (
                "lambda_context" in inspect.signature(self.handler).parameters
            )
...
class BasePartialBatchProcessor(BasePartialProcessor):  # noqa
    ...
    def _register_model_validation_error_record(self, record: dict):
        ...
        # see https://github.com/awslabs/aws-lambda-powertools-python/issues/2091
        logger.debug(
            "Record cannot be converted to customer's model; converting without model"
        )
        failed_record: "EventSourceDataClassTypes" = self._to_batch_type(
            record=record, event_type=self.event_type
        )
...
class AsyncBatchProcessor(BasePartialBatchProcessor):
    ...
    * Sync record handler not supported, use BatchProcessor instead.
    ...
    async def _async_process_record(
        self, record: dict
        ...
        try:
            ...
            if self._handler_accepts_lambda_context:
                result = await self.handler(
                    record=data, lambda_context=self.lambda_context
                )
            else:
