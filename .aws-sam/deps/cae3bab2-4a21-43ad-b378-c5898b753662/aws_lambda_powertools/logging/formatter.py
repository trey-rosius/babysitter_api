...
class LambdaPowertoolsFormatter(BasePowertoolsFormatter):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.json_indent = (
            constants.PRETTY_INDENT
            if powertools_dev_is_set()
            else constants.COMPACT_INDENT
    ...
    def format(self, record: logging.LogRecord) -> str:  # noqa: A003
        ...
        extracted_exception, extracted_exception_name = self._extract_log_exception(
            log_record=record
        )
        formatted_log["exception"] = formatted_log.get("exception", extracted_exception)
        formatted_log["exception_name"] = formatted_log.get(
            "exception_name", extracted_exception_name
        )
    ...
    def formatTime(
        ...
        if self.use_rfc3339_iso8601:
            ...
            return ts_as_datetime.isoformat(
        ...
        if (
            datefmt is None
        ):  # pragma: no cover, it'll always be None in std logging, but mypy
...
RESERVED_FORMATTER_CUSTOM_KEYS: List[str] = inspect.getfullargspec(
