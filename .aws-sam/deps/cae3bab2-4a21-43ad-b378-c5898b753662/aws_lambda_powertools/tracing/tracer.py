...
aws_xray_sdk = LazyLoader(
    constants.XRAY_SDK_MODULE, globals(), constants.XRAY_SDK_MODULE
)
    ...
    * Async handler not supported
    ...
    def capture_lambda_handler(
        ...
        **Preventing Tracer to log response as metadata**
        ...
        def decorate(event, context, **kwargs):
            with self.provider.in_subsegment(
                name=f"## {lambda_handler_name}"
            ) as subsegment:
                try:
    ...
    def capture_method(
        ...
        **Safely tracing each concurrent async calls with escape hatch**
        ...
        elif hasattr(method, "__wrapped__") and inspect.isgeneratorfunction(
            method.__wrapped__
        ):
    ...
    def _decorate_async_function(
        ...
        async def decorate(*args, **kwargs):
            async with self.provider.in_subsegment_async(
                name=f"## {method_name}"
            ) as subsegment:
                try:
    ...
    def _add_response_as_metadata(
        ...
        subsegment.put_metadata(
            key=f"{method_name} response", value=data, namespace=self.service
        )
    ...
    def _add_full_exception_as_metadata(
        ...
        subsegment.put_metadata(
            key=f"{method_name} error", value=error, namespace=self.service
        )
    ...
    def _is_tracer_disabled() -> Union[bool, str]:
        ...
        is_disabled = resolve_truthy_env_var_choice(
    ...
    def __build_config(
        ...
        is_service = resolve_env_var_choice(
        ...
        self._config["provider"] = (
            provider or self._config["provider"] or self._patch_xray_provider()
        )
        self._config["auto_patch"] = (
            auto_patch if auto_patch is not None else self._config["auto_patch"]
        )
    ...
    def ignore_endpoint(
        self, hostname: Optional[str] = None, urls: Optional[List[str]] = None
    ):
