...
class ApiGatewayResolver(BaseRouter):
    ...
    def __init__(
        ...
        self.context: Dict = (
            {}
        )  # early init as customers might add context before event resolution
    ...
    def _not_found(self, method: str) -> ResponseBuilder:
        ...
        if self._cors:
            ...
            headers.update(
                self._cors.to_dict(self.current_event.get_header_value("Origin"))
            )
            ...
            if method == "OPTIONS":
                ...
                return ResponseBuilder(
                    Response(
                        status_code=204, content_type=None, headers=headers, body=""
                    )
                )
...
class APIGatewayRestResolver(ApiGatewayResolver):
    ...
    def __init__(
        ...
        super().__init__(
            ProxyEventType.APIGatewayProxyEvent, cors, debug, serializer, strip_prefixes
        )
    ...
    @staticmethod
    def _compile_regex(rule: str, base_regex: str = _ROUTE_REGEX):
        return super(APIGatewayRestResolver, APIGatewayRestResolver)._compile_regex(
            rule, "^{}/*$"
        )
...
class ALBResolver(ApiGatewayResolver):
    ...
    def __init__(
        ...
        super().__init__(
            ProxyEventType.ALBEvent, cors, debug, serializer, strip_prefixes
        )
