...
def powertools_debug_is_set() -> bool:
    ...
    if is_on:
        warnings.warn(
            "POWERTOOLS_DEBUG environment variable is enabled. Setting logging level to DEBUG.",
            stacklevel=2,
        )
        return True
...
def slice_dictionary(data: Dict, chunk_size: int) -> Generator[Dict, None, None]:
    for _ in range(0, len(data), chunk_size):
        yield {
            dict_key: data[dict_key] for dict_key in itertools.islice(data, chunk_size)
        }
