...
class IdempotencyHandler:
    ...
    def __init__(
        ...
        persistence_store.configure(
            config, f"{self.function.__module__}.{self.function.__qualname__}"
        )
    ...
    def _get_idempotency_record(self) -> DataRecord:
        ...
        # clients
            ...
            raise IdempotencyPersistenceLayerError(
                "Failed to get record from idempotency store", exc
            ) from exc
    ...
    def _handle_for_status(self, data_record: DataRecord) -> Optional[Dict[Any, Any]]:
        ...
        if data_record.status == STATUS_CONSTANTS["INPROGRESS"]:
            if (
                data_record.in_progress_expiry_timestamp is not None
                and data_record.in_progress_expiry_timestamp
