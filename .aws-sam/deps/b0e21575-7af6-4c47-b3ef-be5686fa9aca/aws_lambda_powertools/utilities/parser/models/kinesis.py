...
class KinesisDataStreamRecordPayload(BaseModel):
    ...
    sequenceNumber: str
    data: Union[
        bytes, Type[BaseModel], BaseModel
    ]  # base64 encoded str is parsed into bytes
...
def extract_cloudwatch_logs_from_event(
    event: KinesisDataStreamModel,
) -> List[CloudWatchLogsDecode]:
    return [
        CloudWatchLogsDecode(**record.decompress_zlib_record_data_as_json())
        for record in event.Records
    ]


def extract_cloudwatch_logs_from_record(
    record: KinesisDataStreamRecord,
) -> CloudWatchLogsDecode:
