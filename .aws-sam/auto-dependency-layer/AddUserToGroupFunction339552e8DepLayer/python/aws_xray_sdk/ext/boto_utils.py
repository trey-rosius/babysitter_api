...
whitelist = json.loads(
    pkgutil.get_data(__name__, "resources/aws_para_whitelist.json").decode("utf-8")
)
...
def aws_meta_processor(
    ...
    if return_value:
        ...
        if resp_meta:
            ...
            subsegment.put_http_meta(http.STATUS, resp_meta.get("HTTPStatusCode"))
            # for service like S3 that returns special request id in response headers
            if "HTTPHeaders" in resp_meta and resp_meta["HTTPHeaders"].get(
                "x-amz-id-2"
            ):
    ...
    _extract_whitelisted_params(
        subsegment.name, operation_name, aws_meta, args, kwargs, return_value
    )
...
def _extract_whitelisted_params(service, operation, aws_meta, args, kwargs, response):
    ...
    if "request_descriptors" in params:
        _record_special_params(params["request_descriptors"], args[1], aws_meta)
    ...
    if "response_descriptors" in params and response:
        _record_special_params(params["response_descriptors"], response, aws_meta)
