...
class DefaultSampler(object):
    ...
    def __init__(self):
        ...
        self._target_poller = TargetPoller(
            self._cache, self._rule_poller, self._connector
        )
    ...
    def should_trace(self, sampling_req=None):
        ...
        if matched_rule:
            log.debug(
                "Rule %s is selected to make a sampling decision.", matched_rule.name
            )
    ...
    def load_settings(self, daemon_config, context, origin=None):
        ...
        as the emitter.
        """
        self._connector.setup_xray_client(
            ip=daemon_config.tcp_ip,
