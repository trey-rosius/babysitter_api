...
class KafkaEnvelope(BaseEnvelope):
    ...
    def parse(
        ...
        model_parse_event: Union[
            Type[KafkaMskEventModel], Type[KafkaSelfManagedEventModel]
        ] = (
            KafkaMskEventModel
            if event_source == "aws:kafka"
            else KafkaSelfManagedEventModel
        )

        logger.debug(
            f"Parsing incoming data with Kafka event model {model_parse_event}"
        )
        ...
        for records in parsed_envelope.records.values():
            ret_list += [
                self._parse(data=record.value, model=model) for record in records
            ]
        return ret_list
