...
def _include_registered_loggers_filter(loggers: Set[str]):
    return [
        ...
        if "." not in name and name in loggers
    ]
...
def _exclude_registered_loggers_filter(loggers: Set[str]) -> List[logging.Logger]:
    return [
        logging.getLogger(name)
        for name in logging.root.manager.loggerDict
        if "." not in name and name not in loggers
    ]
...
def _configure_logger(
    source_logger: Logger, logger: logging.Logger, level: Union[int, str]
) -> None:
    logger.handlers = []
    logger.setLevel(level)
    logger.propagate = (
        False  # ensure we don't propagate logs to existing loggers, #1073
    )
