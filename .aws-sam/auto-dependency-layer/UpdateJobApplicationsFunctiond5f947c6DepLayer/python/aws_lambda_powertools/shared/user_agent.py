...
HEADER_NO_OP = (
    f"{FEATURE_PREFIX}/{DEFAULT_FEATURE}/{powertools_version} PTEnv/{EXEC_ENV}"
)
...
def _create_feature_function(feature):
    ...
    def add_powertools_feature(request, **kwargs):
        try:
            ...
            header_user_agent = f"{headers['User-Agent']} {FEATURE_PREFIX}/{feature}/{powertools_version} PTEnv/{EXEC_ENV}"
...
def register_feature_to_resource(resource, feature):
    ...
    try:
        resource.meta.client.meta.events.register(
            TARGET_SDK_EVENT, _create_feature_function(feature)
        )
    except AttributeError as e:
