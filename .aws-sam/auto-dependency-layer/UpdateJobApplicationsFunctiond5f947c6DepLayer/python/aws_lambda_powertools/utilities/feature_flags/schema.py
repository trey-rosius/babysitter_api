...
class RuleAction(str, Enum):
    ...
    SCHEDULE_BETWEEN_TIME_RANGE = (
        "SCHEDULE_BETWEEN_TIME_RANGE"  # hour:min 24 hours clock
    )
    SCHEDULE_BETWEEN_DATETIME_RANGE = (
        "SCHEDULE_BETWEEN_DATETIME_RANGE"  # full datetime format, excluding timezone
    )
    SCHEDULE_BETWEEN_DAYS_OF_WEEK = (
        "SCHEDULE_BETWEEN_DAYS_OF_WEEK"  # MONDAY, TUESDAY, .... see TimeValues enum
    )
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.schema, dict):
            raise SchemaValidationError(
                f"Features must be a dictionary, schema={str(self.schema)}"
            )
...
class FeaturesValidator(BaseValidator):
    ...
    def __init__(
        self, schema: Dict, logger: Optional[Union[logging.Logger, Logger]] = None
    ):
        self.schema = schema
        self.logger = logger or logging.getLogger(__name__)
    ...
    def validate(self):
        for name, feature in self.schema.items():
            ...
            rules = RulesValidator(
                feature=feature, boolean_feature=boolean_feature, logger=self.logger
            )
            rules.validate()
    ...
    def validate_feature(name, feature) -> bool:
        ...
        if default_value is None or (
            not isinstance(default_value, bool) and boolean_feature
        ):
            raise SchemaValidationError(
...
class RulesValidator(BaseValidator):
    ...
    def validate(self):
        ...
        if not isinstance(self.rules, dict):
            self.logger.debug(
                f"Feature rules must be a dictionary, feature={self.feature_name}"
            )
            raise SchemaValidationError(
                f"Feature rules must be a dictionary, feature={self.feature_name}"
            )
        ...
        for rule_name, rule in self.rules.items():
            self.logger.debug(
                f"Attempting to validate rule={rule_name} and feature={self.feature_name}"
            )
            self.validate_rule(
                rule=rule,
                rule_name=rule_name,
            ...
            conditions = ConditionsValidator(
                rule=rule, rule_name=rule_name, logger=self.logger
            )
            conditions.validate()

    @staticmethod
    def validate_rule(
        rule: Dict, rule_name: str, feature_name: str, boolean_feature: bool = True
    ):
        if not rule or not isinstance(rule, dict):
            raise SchemaValidationError(
                f"Feature rule must be a dictionary, feature={feature_name}"
            )

        RulesValidator.validate_rule_name(
            rule_name=rule_name, feature_name=feature_name
        )
        RulesValidator.validate_rule_default_value(
            rule=rule, rule_name=rule_name, boolean_feature=boolean_feature
        )
    ...
    def validate_rule_name(rule_name: str, feature_name: str):
        if not rule_name or not isinstance(rule_name, str):
            raise SchemaValidationError(
                f"Rule name key must have a non-empty string, feature={feature_name}"
            )
    ...
    def validate_rule_default_value(rule: Dict, rule_name: str, boolean_feature: bool):
        ...
        if boolean_feature and not isinstance(rule_default_value, bool):
            raise SchemaValidationError(
                f"'rule_default_value' key must have be bool, rule={rule_name}"
            )
...
class ConditionsValidator(BaseValidator):
    ...
    def validate(self):
        if not self.conditions or not isinstance(self.conditions, list):
            self.logger.debug(
                f"Condition is empty or invalid for rule={self.rule_name}"
            )
    ...
    def validate_condition(rule_name: str, condition: Dict[str, str]) -> None:
        if not condition or not isinstance(condition, dict):
            raise SchemaValidationError(
                f"Feature rule condition must be a dictionary, rule={rule_name}"
            )

        ConditionsValidator.validate_condition_action(
            condition=condition, rule_name=rule_name
        )
        ConditionsValidator.validate_condition_key(
            condition=condition, rule_name=rule_name
        )
        ConditionsValidator.validate_condition_value(
            condition=condition, rule_name=rule_name
        )
    ...
    def validate_condition_key(condition: Dict[str, Any], rule_name: str):
        ...
        if not key or not isinstance(key, str):
            raise SchemaValidationError(
                f"'key' value must be a non empty string, rule={rule_name}"
            )
        ...
        if (
            ...
            and key != TimeKeys.CURRENT_TIME.value
        ):
            raise SchemaValidationError(
        ...
        if (
            ...
            and key != TimeKeys.CURRENT_DATETIME.value
        ):
            raise SchemaValidationError(
        ...
        if (
            ...
            and key != TimeKeys.CURRENT_DAY_OF_WEEK.value
        ):
            raise SchemaValidationError(
    ...
    def validate_condition_value(condition: Dict[str, Any], rule_name: str):
        ...
        if value is None:
            raise SchemaValidationError(
                f"'value' key must not be null, rule={rule_name}"
            )
        action = condition.get(CONDITION_ACTION, "")
        ...
        if action == RuleAction.SCHEDULE_BETWEEN_TIME_RANGE.value:
            ...
            ConditionsValidator._validate_schedule_between_days_of_week(
                value, rule_name
            )
    ...
    def _validate_datetime_value(datetime_str: str, rule_name: str):
        ...
        try:
            ...
            raise SchemaValidationError(
                f"'START' and 'END' must be a valid ISO8601 time format, rule={rule_name}"
            )
    ...
    def _validate_schedule_between_days_of_week(value: Any, rule_name: str):
        ...
        if not tz.gettz(timezone):
            raise SchemaValidationError(
                f"'TIMEZONE' value must represent a valid IANA timezone, rule={rule_name}"
            )
    ...
    def _validate_schedule_between_time_and_datetime_ranges(
        ...
        if not isinstance(start_time, str) or not isinstance(end_time, str):
            raise SchemaValidationError(
                f"'START' and 'END' must be a non empty string, rule={rule_name}"
            )
        ...
        if not isinstance(timezone, str):
            raise SchemaValidationError(
                f"'TIMEZONE' must be a string, rule={rule_name}"
            )
