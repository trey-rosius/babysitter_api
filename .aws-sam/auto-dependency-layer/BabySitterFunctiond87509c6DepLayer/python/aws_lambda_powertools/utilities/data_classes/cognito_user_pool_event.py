...
class CallerContext(DictWrapper):
    ...
    def client_id(self) -> str:
        """The ID of the client associated with the users pool."""
...
class BaseTriggerEvent(DictWrapper):
    ...
    def user_pool_id(self) -> str:
        """The users pool ID for the users pool."""
    ...
    def user_name(self) -> str:
        """The username of the current users."""
...
class PreSignUpTriggerEventRequest(DictWrapper):
    ...
    def user_attributes(self) -> Dict[str, str]:
        """One or more name-value pairs representing users attributes. The attribute names are the keys."""
        return self["request"]["userAttributes"]

    @property
    def validation_data(self) -> Optional[Dict[str, str]]:
        """One or more name-value pairs containing the validation data in the request to register a users."""
...
class PreSignUpTriggerEventResponse(DictWrapper):
    ...
    def auto_confirm_user(self, value: bool):
        """Set to true to auto-confirm the users, or false otherwise."""
    ...
    def auto_verify_email(self, value: bool):
        """Set to true to set as verified the email of a users who is signing up, or false otherwise."""
    ...
    def auto_verify_phone(self, value: bool):
        """Set to true to set as verified the phone number of a users who is signing up, or false otherwise."""
...
class PreSignUpTriggerEvent(BaseTriggerEvent):
    ...
    - `PreSignUp_AdminCreateUser` Pre sign-up when an admin creates a new users.
...
class UserMigrationTriggerEventResponse(DictWrapper):
    ...
    def user_attributes(self, value: Dict[str, str]):
        """It must contain one or more name-value pairs representing users attributes to be stored in the
        users profile in your users pool. You can include both standard and custom users attributes.
    ...
    def final_user_status(self, value: str):
        ...
        allow them to sign-in with their previous passwords. This is the simplest experience for the users.

        If this attribute is set to RESET_REQUIRED, the users is required to change his or her password immediately
    ...
    def force_alias_creation(self, value: bool):
        ...
        parameter already exists as an alias with a different users, the API call will migrate the alias from the
        previous users to the newly created users. The previous users will no longer be able to log in using that alias.

        If this attribute is set to "false" and the alias exists, the users will not be migrated, and an error is
...
class CustomMessageTriggerEventRequest(DictWrapper):
    ...
    def username_parameter(self) -> str:
        """The username parameter. It is a required request parameter for the admin create users flow."""
...
class CustomMessageTriggerEvent(BaseTriggerEvent):
    ...
    - `CustomMessage_AdminCreateUser` To send the temporary password to a new users.
    - `CustomMessage_ResendCode` To resend the confirmation code to an existing users.
    - `CustomMessage_ForgotPassword` To send the confirmation code for Forgot Password request.
    - `CustomMessage_UpdateUserAttribute` When a users's email or phone number is changed, this trigger sends a
       verification code automatically to the users. Cannot be used for other attributes.
    - `CustomMessage_VerifyUserAttribute`  This trigger sends a verification code to the users when they manually
...
class PreAuthenticationTriggerEventRequest(DictWrapper):
    ...
    @property
    def user_attributes(self) -> Dict[str, str]:
        """One or more name-value pairs representing users attributes."""
        return self["request"]["userAttributes"]

    @property
    def validation_data(self) -> Optional[Dict[str, str]]:
        """One or more key-value pairs containing the validation data in the users's sign-in request."""
        return self["request"].get("validationData")
...
class PreAuthenticationTriggerEvent(BaseTriggerEvent):
    ...
    Amazon Cognito invokes this trigger when a users attempts to sign in, allowing custom validation
...
class PostAuthenticationTriggerEventRequest(DictWrapper):
    ...
    def new_device_used(self) -> bool:
        """This flag indicates if the users has signed in on a new device.
        It is set only if the remembered devices value of the users pool is set to `Always` or User `Opt-In`."""
...
class PostAuthenticationTriggerEvent(BaseTriggerEvent):
    ...
    Amazon Cognito invokes this trigger after signing in a users, allowing you to add custom logic
...
class GroupOverrideDetails(DictWrapper):
    ...
    def groups_to_override(self) -> Optional[List[str]]:
        """A list of the group names that are associated with the users that the identity token is issued for."""
...
class PreTokenGenerationTriggerEvent(BaseTriggerEvent):
    ...
    - `TokenGeneration_Authentication` Called after users authentication flows have completed.
    - `TokenGeneration_NewPasswordChallenge` Called after the users is created by an admin. This flow is invoked
       when the users has to change a temporary password.
    - `TokenGeneration_AuthenticateDevice` Called at the end of the authentication of a users device.
    - `TokenGeneration_RefreshTokens` Called when a users tries to refresh the identity and access tokens.
...
class ChallengeResult(DictWrapper):
    ...
    def challenge_result(self) -> bool:
        """Set to true if the users successfully completed the challenge, or false otherwise."""
...
class DefineAuthChallengeTriggerEventRequest(DictWrapper):
    ...
    @property
    def user_not_found(self) -> Optional[bool]:
        """A Boolean that is populated when PreventUserExistenceErrors is set to ENABLED for your users pool client.
        A value of true means that the users id (users name, email address, etc.) did not match any existing users."""
        return self["request"].get("userNotFound")
...
class DefineAuthChallengeTriggerEventResponse(DictWrapper):
    ...
    def challenge_name(self, value: str):
        ...
        If you want to present a new challenge to your users, specify the challenge name here."""
    ...
    def issue_tokens(self, value: bool):
        """Set to true if you determine that the users has been sufficiently authenticated by
...
class CreateAuthChallengeTriggerEventResponse(DictWrapper):
    ...
    def public_challenge_parameters(self, value: Dict[str, str]):
        """One or more key-value pairs for the client app to use in the challenge to be presented to the users.
    ...
    def private_challenge_parameters(self, value: Dict[str, str]):
        ...
        This parameter should contain all of the information that is required to validate the users's
        response to the challenge. In other words, the publicChallengeParameters parameter contains the
        question that is presented to the users and privateChallengeParameters contains the valid answers
        for the question."""
...
class VerifyAuthChallengeResponseTriggerEventRequest(DictWrapper):
    ...
    def private_challenge_parameters(self) -> Dict[str, str]:
        ...
        compared against a users’s challengeAnswer to determine whether the users passed the challenge."""
        return self["request"]["privateChallengeParameters"]

    @property
    def challenge_answer(self) -> Any:
        """The answer from the users's response to the challenge."""
...
class VerifyAuthChallengeResponseTriggerEventResponse(DictWrapper):
    ...
    def answer_correct(self, value: bool):
        """Set to true if the users has successfully completed the challenge, or false otherwise."""
...
class VerifyAuthChallengeResponseTriggerEvent(BaseTriggerEvent):
    ...
    Amazon Cognito invokes this trigger to verify if the response from the end users for a custom
    Auth Challenge is valid or not.
    It is part of a users pool custom authentication flow.
