    ...
    def _build_boto3_resource_client(
        ...
        if client is not None:
            user_agent.register_feature_to_resource(
                resource=client, feature="parameters"
            )
            return client
        ...
        client = session.resource(
            service_name=service_name, config=config, endpoint_url=endpoint_url
        )
...
def get_transform_method(
    ...
    if transform == "auto":
        ...
        transform_method = TRANSFORM_METHOD_MAPPING.get(
            key_suffix, TRANSFORM_METHOD_MAPPING[None]
        )

    return cast(
        Callable, transform_method
            ...
            try:
                ...
                if raise_on_transform_error:
                    raise TransformParameterError(
                        err_msg.format(transform=transform, exc=exc)
                    ) from exc
